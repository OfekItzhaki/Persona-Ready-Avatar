...
export async function GET() {
  ...
  try {
    ...
    // Brain API returns array directly, but frontend expects {agents: [...]}
    // Wrap the array if it's not already wrapped
    ...
    if (Array.isArray(agents)) {
      agents = agents.map(
        (agent: { voice?: string; language?: string; [key: string]: unknown }) => ({
          ...agent,
